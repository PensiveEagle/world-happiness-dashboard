...
def generate_scatter_plot( data: pd.DataFrame, x_column: str, y_column: str ) -> plotly_figure:
    plot =px.scatter( data_frame = data, x = data[x_column], y = data[y_column], labels = { "x": x_column, "y": y_column } )
    return plot
    
if __name__ == "__main__":
    import streamlit as st
    line_chart = generate_line_plot( data_functions.get_data_from_csv(), "happiness", "gdp" )#
    scatter_chart = generate_scatter_plot( data_functions.get_data_from_csv(), "happiness", "gdp" )
    st.title( "test chart" )
